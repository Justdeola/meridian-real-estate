    ...
    <header className="sticky top-0 z-40 border-b border-line bg-bg/90 backdrop-blur-md">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6">
        ...
        <div className="flex items-center gap-2">
          {isPending ? (
            ...
            <>
              <SignedIn>
                <Link
                  to="/favorites"
                  className="relative grid size-11 place-items-center rounded-[10px] hover:bg-ink/5"
          ...
          <button
            type="button"
            className="grid size-11 place-items-center rounded-[10px] lg:hidden"
            onClick={() => setOpen((v) => !v)}
            aria-label="Menu"
          >
      ...
      <div
        className={cn(
          "overflow-hidden border-t border-line bg-bg transition-[max-height,opacity] duration-250 ease-[cubic-bezier(0.22,1,0.36,1)] lg:hidden",
          open ? "max-h-96 opacity-100" : "max-h-0 opacity-0",
        )}
      >
        <div className="flex flex-col gap-1 px-4 py-4">
          {NAV.map((item) => (
            <Link key={item.label} to={item.to} search={item.search} className="py-3 text-base">
              {item.label}
            </Link>
          ))}
          <Link to="/properties" className="py-3 text-base">
            All listings
          </Link>
        </div>
      </div>
